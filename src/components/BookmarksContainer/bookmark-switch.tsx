...
export const BookmarkSwitch = ({ id, indentLevel = 0, hideDetails = false }: BookmarkSwitchProps) => {
  const bookmark = useBookmark(id);
  if (bookmark.children) {
    return <Folder key={id} id={id} indentLevel={indentLevel} hideDetails={hideDetails} />;
  } else {
